...
def get_data(set):
    ...
    if set == 'abalone':
        ...
        dat = pd.read_csv(path + 'abalone.data', names=colnames)
        # changing class to non numeric
        dat['Rings'] = dat['Rings'].astype(str)
        # sex = {'I':0,'F':1,'M':2}
        # dat['Sex'] = dat['Sex'].map(sex)
        # no missing data per NAMES file

    # car data ---------------------------
    if set == 'car':
        colnames = ['buying', 'maint', 'doors', 'persons', 'lug_boot', 'saftey', 'Class']
        dat = pd.read_csv(path + 'car.data', names=colnames)
        # no missing data per NAMES file
    ...
    if set == 'wine':
        ...
        'Nonflavanoid_phenols', 'Proanthocyanins', 'Color_Intensity', 'Hue', 'OD280/0D315_of_diluted_wines', 'Proline']
        dat = pd.read_csv(path + 'wine.data', names=colnames)
        # no missing data per NAMES file
